...
**"임시중단/마감"** → **"신청 가능"** 으로 바뀌는 순간을 **매시 15분 단위(00, 15, 30, 45분)** 감지해서
...
| `CASPER_ALIGN_TO_HOUR` | `1` | `1`이면 정각 정렬(`ALIGN_MINUTES` 간격)로 체크. `0`이면 `INTERVAL_SEC` 간격 |
| `CASPER_ALIGN_MINUTES` | `15` | 정각 정렬 간격(분). `15`=00/15/30/45분, `60`=매 정시각(HH:00) |
